...
int main(int argc, char** argv)
{
	...
	SNDFILE *opened_file;
	if (argc<3){
		printf("Not enough arguments. First, type in the window size. The name of the file should follow.\n");
		printf("Format)) MDCT_converter (window size) (file name)\n");
		return -3;
	}
	++argv;
	window_size=str_to_int(*argv);
	if (window_size%2) {
		printf("Window size must be an even number.\n");
		return -2;
	} 
	...
	for (i=0; i<soundinfo.frames; ++i)/*입력받은 PCM 신호를 각 채널로 분리.*/
	{
		...
		{
			pcmch[j][i+window_size>>1]=pcmaud[i*2+j];
		}
	}
